...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('cpf', 'matricula', 'banco_emprestimo', 'ref_month', 'ref_year',
                          name='uix_siape_unique_ref')
    )

    # Criar índices para siape_lines
    op.create_index(op.f('ix_siape_lines_cpf'), 'siape_lines', ['cpf'], unique=False)
    op.create_index(op.f('ix_siape_lines_matricula'), 'siape_lines', ['matricula'], unique=False)

    # Criar tabela client_siape_info
    op.create_table(
        'client_siape_info',
        sa.Column('id', sa.Integer(), nullable=False),
