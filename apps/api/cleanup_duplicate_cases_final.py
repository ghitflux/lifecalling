"""
Script Final de Limpeza de Casos Duplicados
============================================

Remove casos duplicados por CPF (FINAL).
Mantém apenas 1 caso por CPF - o mais recente.
"""
...
def cleanup_duplicate_cases_final(dry_run=True):
    """
    Remove casos duplicados por CPF (verificação final).
    ...
    try:
        ...
        query = text("""
            SELECT
                cl.cpf,
                STRING_AGG(DISTINCT cl.name, ' / ' ORDER BY cl.name) as nomes,
        ...
        print(f"\n{'='*80}")
        print(f"LIMPEZA FINAL DE CASOS DUPLICADOS - {'DRY RUN' if dry_run else 'EXECUÇÃO REAL'}")
        print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Limpeza final de casos duplicados')
    parser.add_argument('--execute', action='store_true',
                       help='Executar limpeza (sem isso, apenas mostra o que seria deletado)')

    args = parser.parse_args()

    cleanup_duplicate_cases_final(dry_run=not args.execute)
