...
Mantém apenas os 2 meses mais recentes para cada combinação de (cpf, matricula, financiamento_code).
...
def cleanup_old_references(session, dry_run=False):
    ...
    logger.info(f"Linhas a manter (2 meses recentes): {stats_before['to_keep']}")
    logger.info(f"\nDistribuição por mês/ano:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Limpa referências antigas de PayrollLine, mantendo apenas os 2 meses mais recentes."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
