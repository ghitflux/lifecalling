...
export default function Sidebar() {
  ...
  return (
    <aside className={cn(
      "h-screen bg-card/70 border-r border-border/50 backdrop-blur-sm sticky top-0",
      collapsed ? "w-16" : "w-64"
    )}>
      <div className="flex items-center justify-between p-3">
        <div className={cn("text-sm font-semibold", collapsed && "truncate")}>Call Center</div>
        <Button size="sm" variant="ghost" className="h-8 w-8" onClick={()=>setCollapsed(v=>!v)}>
        ...
        {NAV.filter(i => i.roles.includes(role as Role)).map(({label, href, icon:Icon})=>{
          ...
          return (
            <Link key={href} href={href}
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-2 text-sm hover:bg-muted",
                active && "bg-muted"
              )}>
      ...
      <div className="mt-auto p-2">
        ...
        {!collapsed && user && (
          <div className="mt-2 text-xs opacity-70 px-1">
