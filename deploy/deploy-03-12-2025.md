# Deploy - 03/12/2025

## ⚠️ ALERTA CRÍTICO - BACKUP
**Na última atualização houve um problema crítico**: o backup foi criado mas veio com banco de dados vazio!

**PROCEDIMENTO OBRIGATÓRIO**:
1. ✅ Criar backup
2. ✅ **VALIDAR** que o backup contém dados
3. ✅ **TESTAR** restauração em ambiente de teste
4. ✅ Só então prosseguir com o deploy

## Informações Gerais
- **Data**: 03/12/2025
- **Servidor**: 72.60.158.156 (root)
- **Diretório**: /opt/lifeservicos/src (NÃO MUDAR)
- **Branch**: life-system → origin/life-system
- **Commit**: 0d7484f
- **Arquivos modificados**: 6 arquivos (+352 linhas, -87 linhas)

## Resumo das Mudanças

### 1. Otimização de Mensalidades ⚡
- Sistema agora mantém **apenas 1 mensalidade** (a mais recente) por contrato
- Antes: mantinha 2 mensalidades
- **Limpeza executada**: 88,625 linhas removidas (~44 MB)
- Criado script automático de limpeza: `cleanup_old_payroll_data.py`

### 2. Correção de Filtros no Módulo de Atendimento 🔍
- **Filtro por Banco**: Corrigido para aceitar nomes normalizados
- **Filtro por Cargo**: Implementado do zero (não existia)
- **Filtros Cruzados**: Banco + Cargo + Status funcionam simultaneamente
- Problema resolvido: dropdowns não filtravam os cards

### 3. Melhorias de UX - Navegação entre Casos 🚀
- **Removido modal obrigatório** de justificativa ao navegar
- Navegação instantânea entre casos (Próximo/Anterior)
- Usuário pode pular casos sem comentar

## Arquivos Modificados

1. **apps/api/app/routers/cases.py** (+58 linhas)
   - Adicionado filtro por `cargo` (parâmetro novo)
   - Filtro por `entidade` aceita nomes normalizados
   - Busca automática de variações de nomes de banco (ex: "BANCO DO BRASIL S.A" → "BANCO DO BRASIL")

2. **apps/api/app/routers/imports.py** (+10 linhas, -10 linhas)
   - Função `cleanup_old_references()`: muda de `rn > 2` para `rn > 1`
   - Limpeza automática após cada importação de folha

3. **apps/api/cleanup_payroll_references.py** (+10 linhas, -10 linhas)
   - Atualizado para manter apenas 1 mensalidade
   - Documentação atualizada

4. **apps/api/cleanup_old_payroll_data.py** (NOVO - 352 linhas)
   - Script de limpeza única com validação
   - Modo `--dry-run` para simulação
   - Estatísticas detalhadas antes/depois
   - Confirmação obrigatória: usuário deve digitar "CONFIRMO"

5. **apps/web/src/lib/query.ts** (+5 linhas)
   - Tipo `Filters`: adicionado `banco` e `cargo`
   - Função `buildCasesQuery`: processa filtros de banco e cargo
   - `banco` é alias para `entidade` (compatibilidade)

6. **apps/web/src/app/casos/[id]/page.tsx** (-30 linhas)
   - Removido modal de navegação completo
   - Função `handleNavigate` simplificada (não async)
   - Estados removidos: `navigationReason`, `showNavigationDialog`, `navigationDirection`

## Migrations

**Nenhuma migration necessária** - As mudanças usam tabelas e campos existentes.

## Impacto nos Usuários

### Positivo ✅
- Filtros de atendimento funcionam corretamente (banco, cargo, status)
- Navegação entre casos mais rápida e fluida
- Banco de dados mais leve (~44 MB economizados)
- Sistema mais rápido (menos dados para processar)

### Neutro ⚠️
- Mensalidades antigas foram removidas (apenas a mais recente é mantida)
- Para consultar histórico, usar relatórios antes do deploy

### Negativo ❌
- Nenhum impacto negativo identificado

---

## PROCEDIMENTO DE DEPLOY

### ⚠️ ETAPA 0: VALIDAÇÃO PRÉ-DEPLOY (OBRIGATÓRIA)

```bash
# SSH no servidor
ssh root@72.60.158.156

# Verificar diretório correto
cd /opt/lifeservicos/src
pwd
# Esperado: /opt/lifeservicos/src

# Verificar branch atual
git branch
# Esperado: * life-system

# Verificar último commit
git log -1 --oneline
# Esperado: c2d4a71 ou mais recente

# Verificar status dos containers
docker compose -f docker-compose.prod.yml ps
# Todos devem estar "Up"
```

---

### ETAPA 1: BACKUP DO BANCO DE DADOS ⚠️ CRÍTICO

#### 1.1. Criar Backup Completo

```bash
# Criar diretório de backups com data
BACKUP_DIR="/opt/lifeservicos/backups/deploy-$(date +%Y%m%d-%H%M%S)"
mkdir -p $BACKUP_DIR

# Backup formato CUSTOM (comprimido, restaurável)
docker exec src-db-1 pg_dump \
  -U lifecalling \
  -d lifecalling \
  -F custom \
  -f /tmp/backup-custom.backup

# Backup formato PLAIN TEXT (legível, auditável)
docker exec src-db-1 pg_dump \
  -U lifecalling \
  -d lifecalling \
  -F plain \
  > /tmp/backup-plain.sql

# Copiar backups para host
docker cp src-db-1:/tmp/backup-custom.backup $BACKUP_DIR/
docker cp src-db-1:/tmp/backup-plain.sql $BACKUP_DIR/

echo "Backups criados em: $BACKUP_DIR"
```

#### 1.2. ⚠️ VALIDAR BACKUP (OBRIGATÓRIO)

```bash
# Verificar tamanho dos arquivos (devem ser > 100 KB)
ls -lh $BACKUP_DIR/
# backup-custom.backup deve ter ~10-50 MB
# backup-plain.sql deve ter ~20-100 MB

# Contar linhas do backup plain (deve ter milhares)
wc -l $BACKUP_DIR/backup-plain.sql
# Esperado: > 10000 linhas

# Verificar se backup contém dados de tabelas críticas
grep -c "INSERT INTO" $BACKUP_DIR/backup-plain.sql
# Esperado: > 100

grep -c "clients" $BACKUP_DIR/backup-plain.sql
# Esperado: > 50

grep -c "cases" $BACKUP_DIR/backup-plain.sql
# Esperado: > 50

grep -c "payroll_lines" $BACKUP_DIR/backup-plain.sql
# Esperado: > 100

# Verificar se NÃO está vazio
if [ $(wc -l < $BACKUP_DIR/backup-plain.sql) -lt 1000 ]; then
  echo "❌ ERRO: Backup parece estar vazio ou corrompido!"
  echo "❌ NÃO PROSSIGA COM O DEPLOY!"
  exit 1
else
  echo "✅ Backup validado com sucesso"
fi
```

#### 1.3. 🧪 TESTAR RESTAURAÇÃO (ALTAMENTE RECOMENDADO)

```bash
# Criar banco de teste
docker exec src-db-1 psql -U lifecalling -c "CREATE DATABASE lifecalling_test;"

# Restaurar backup no banco de teste
docker cp $BACKUP_DIR/backup-custom.backup src-db-1:/tmp/
docker exec src-db-1 pg_restore \
  -U lifecalling \
  -d lifecalling_test \
  -c \
  /tmp/backup-custom.backup

# Verificar se dados foram restaurados
docker exec src-db-1 psql -U lifecalling -d lifecalling_test -c "
  SELECT 'clients' as table, COUNT(*) as count FROM clients
  UNION ALL
  SELECT 'cases', COUNT(*) FROM cases
  UNION ALL
  SELECT 'payroll_lines', COUNT(*) FROM payroll_lines
  UNION ALL
  SELECT 'contracts', COUNT(*) FROM contracts;
"
# Todas as contagens devem ser > 0

# Se teste OK, remover banco de teste
docker exec src-db-1 psql -U lifecalling -c "DROP DATABASE lifecalling_test;"

echo "✅ Teste de restauração bem-sucedido!"
```

#### 1.4. Limpar Backups Antigos (> 30 dias)

```bash
# Manter backups dos últimos 30 dias
find /opt/lifeservicos/backups -maxdepth 1 -type d -mtime +30 -exec rm -rf {} \;
```

---

### ETAPA 2: ATUALIZAR CÓDIGO NO SERVIDOR

```bash
# Navegar para diretório
cd /opt/lifeservicos/src

# Fazer stash de mudanças locais (se houver)
git stash

# Fetch das mudanças
git fetch origin

# Verificar diferenças antes de fazer pull
git log --oneline HEAD..origin/life-system

# Pull das mudanças
git pull origin life-system

# Verificar commit atual
git log -1 --oneline
# Esperado: 0d7484f feat: Melhorias no sistema de atendimento e mensalidades

# Verificar arquivos modificados
git show --name-only 0d7484f
```

---

### ETAPA 3: PARAR CONTAINERS API E WEB

```bash
cd /opt/lifeservicos/src

# Parar apenas API e Web (mantém DB rodando)
docker compose -f docker-compose.prod.yml stop api web

# Verificar status
docker compose -f docker-compose.prod.yml ps
# api e web devem estar "Exited"
# db deve estar "Up"
```

---

### ETAPA 4: REBUILD DOS CONTAINERS (SE NECESSÁRIO)

```bash
# Para mudanças apenas em Python (.py) e TypeScript (.tsx, .ts):
# NÃO é necessário rebuild, basta restart

# Verificar se há mudanças em Dockerfile ou requirements.txt
git diff HEAD~1 --name-only | grep -E 'Dockerfile|requirements.txt|package.json'

# Se houver mudanças, fazer rebuild:
docker compose -f docker-compose.prod.yml build api web

# Caso contrário, pular para ETAPA 5
```

---

### ETAPA 5: REINICIAR CONTAINERS

```bash
cd /opt/lifeservicos/src

# Reiniciar API e Web
docker compose -f docker-compose.prod.yml up -d api web

# Aguardar inicialização (20-30 segundos)
sleep 30

# Verificar status
docker compose -f docker-compose.prod.yml ps
# Todos devem estar "Up" e "healthy"

# Verificar logs iniciais
docker compose -f docker-compose.prod.yml logs --tail=50 api | grep -i error
# Não deve haver erros críticos
```

---

### ETAPA 6: VERIFICAÇÃO FUNCIONAL

#### 6.1. Verificar API

```bash
# Health check
curl -s https://api.lifeservicos.com/health
# Esperado: {"status":"ok","message":"Lifecalling API está funcionando"}

# Verificar se filtros estão disponíveis
curl -s https://api.lifeservicos.com/clients/filters | jq '.bancos | length'
# Esperado: > 0

curl -s https://api.lifeservicos.com/clients/filters | jq '.cargos | length'
# Esperado: > 0
```

#### 6.2. Verificar Banco de Dados

```bash
# Verificar tabela payroll_lines (após limpeza)
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT
    ref_year,
    ref_month,
    COUNT(*) as total
  FROM payroll_lines
  GROUP BY ref_year, ref_month
  ORDER BY ref_year DESC, ref_month DESC
  LIMIT 5;
"
# Deve mostrar distribuição de mensalidades (mais recente com mais linhas)

# Verificar se há duplicatas (não deve ter)
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT
    cpf,
    matricula,
    financiamento_code,
    COUNT(*) as duplicates
  FROM payroll_lines
  GROUP BY cpf, matricula, financiamento_code
  HAVING COUNT(*) > 1
  LIMIT 10;
"
# Esperado: 0 resultados (sem duplicatas)

# Verificar total de linhas
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT COUNT(*) FROM payroll_lines;
"
# Esperado: ~109,265 linhas (após limpeza)
```

#### 6.3. Verificar Frontend

```bash
# Acessar site
curl -s https://lifeservicos.com | grep -q "Life Digital"
echo $?
# Esperado: 0 (site carregou)

# Verificar build
ls -lh /opt/lifeservicos/src/apps/web/.next/
# Deve ter arquivos recentes
```

---

### ETAPA 7: TESTE FUNCIONAL MANUAL

#### 7.1. Testar Filtros de Atendimento

1. Acessar: https://lifeservicos.com/esteira
2. Selecionar filtro **BANCO** (ex: "BANCO DO BRASIL")
3. Verificar que cards filtrados aparecem corretamente
4. Selecionar filtro **CARGO** (ex: "007-DIRETOR")
5. Verificar que filtros cruzados funcionam
6. Clicar em **Limpar filtros**
7. Verificar que todos os casos voltam a aparecer

✅ **Sucesso**: Filtros funcionam corretamente

#### 7.2. Testar Navegação entre Casos

1. Acessar um caso: https://lifeservicos.com/casos/46724
2. Clicar no botão **Próximo**
3. Verificar que navega diretamente (sem modal)
4. Clicar no botão **Anterior**
5. Verificar que volta ao caso anterior

✅ **Sucesso**: Navegação instantânea sem modal

#### 7.3. Testar Importação de Folha (Limpeza Automática)

1. Acessar: https://lifeservicos.com/importacao
2. Importar arquivo de folha de pagamento
3. Aguardar conclusão
4. Verificar logs: `docker logs src-api-1 --tail=100 | grep "Limpeza"`
5. Deve aparecer: "Limpeza concluída: X linhas antigas removidas"

✅ **Sucesso**: Limpeza automática funcionando

---

## MONITORAMENTO PÓS-DEPLOY

### Métricas a Observar (primeiras 24h)

```bash
# 1. Logs de erro
docker logs src-api-1 --tail=500 | grep -i error

# 2. Uso de CPU/Memória
docker stats --no-stream src-api-1 src-web-1 src-db-1

# 3. Espaço em disco (banco menor)
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT pg_size_pretty(pg_database_size('lifecalling'));
"

# 4. Tempo de resposta da API
time curl -s https://api.lifeservicos.com/health

# 5. Queries lentas no PostgreSQL
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time
  FROM pg_stat_statements
  ORDER BY mean_exec_time DESC
  LIMIT 10;
"
```

### Alertas

Se houver:
- ❌ Erros 500 na API → Verificar logs
- ❌ Timeout em queries → Verificar índices
- ❌ Uso alto de memória → Verificar leaks
- ❌ Duplicatas em payroll_lines → Executar limpeza manual

---

## ROLLBACK (SE NECESSÁRIO)

### Cenário 1: Problema com Código

```bash
# Reverter código
cd /opt/lifeservicos/src
git reset --hard c2d4a71  # commit anterior

# Reiniciar containers
docker compose -f docker-compose.prod.yml restart api web

# Verificar
curl -s https://api.lifeservicos.com/health
```

### Cenário 2: Problema com Banco de Dados

```bash
# PARAR TODOS OS CONTAINERS
docker compose -f docker-compose.prod.yml stop

# Restaurar backup
BACKUP_DIR="/opt/lifeservicos/backups/deploy-YYYYMMDD-HHMMSS"  # Usar o mais recente

# Limpar banco atual
docker exec src-db-1 psql -U lifecalling -c "DROP DATABASE lifecalling;"
docker exec src-db-1 psql -U lifecalling -c "CREATE DATABASE lifecalling;"

# Restaurar backup custom
docker cp $BACKUP_DIR/backup-custom.backup src-db-1:/tmp/
docker exec src-db-1 pg_restore \
  -U lifecalling \
  -d lifecalling \
  -c \
  /tmp/backup-custom.backup

# Verificar restauração
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT COUNT(*) FROM clients;
  SELECT COUNT(*) FROM cases;
  SELECT COUNT(*) FROM payroll_lines;
"

# Reiniciar containers
docker compose -f docker-compose.prod.yml up -d

# Verificar
curl -s https://api.lifeservicos.com/health
```

---

## SCRIPTS AUXILIARES

### Script: Verificar Duplicatas em Mensalidades

```bash
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT
    cpf,
    matricula,
    financiamento_code,
    COUNT(*) as total,
    STRING_AGG(ref_month::text || '/' || ref_year::text, ', ') as referencias
  FROM payroll_lines
  GROUP BY cpf, matricula, financiamento_code
  HAVING COUNT(*) > 1
  ORDER BY total DESC
  LIMIT 20;
"
```

### Script: Executar Limpeza Manual (se necessário)

```bash
# SSH no servidor
ssh root@72.60.158.156

# Entrar no container API
docker exec -it src-api-1 bash

# Executar script de limpeza (modo simulação)
cd /app/apps/api
python cleanup_old_payroll_data.py --dry-run

# Se OK, executar limpeza real
python cleanup_old_payroll_data.py
# Digite: CONFIRMO

# Verificar resultado
exit

# Verificar no banco
docker exec src-db-1 psql -U lifecalling -d lifecalling -c "
  SELECT COUNT(*) FROM payroll_lines;
"
```

---

## CHECKLIST FINAL

### Antes do Deploy
- [ ] Backup criado e salvo em $BACKUP_DIR
- [ ] Backup validado (tamanho > 100 KB, linhas > 10000)
- [ ] Teste de restauração executado e bem-sucedido
- [ ] Código atualizado (git pull)
- [ ] Commit correto verificado (0d7484f)

### Durante o Deploy
- [ ] Containers parados (api, web)
- [ ] Código atualizado
- [ ] Containers reiniciados
- [ ] Logs sem erros críticos

### Após o Deploy
- [ ] Health check OK
- [ ] Filtros de banco e cargo funcionando
- [ ] Navegação entre casos sem modal
- [ ] Importação de folha com limpeza automática
- [ ] Sem duplicatas em payroll_lines
- [ ] Monitoramento ativo (24h)

---

## CONTATOS DE EMERGÊNCIA

- **Servidor**: root@72.60.158.156
- **Diretório**: /opt/lifeservicos/src
- **GitHub**: https://github.com/ghitflux/lifeservicos
- **Branch**: life-system
- **Backups**: /opt/lifeservicos/backups/

---

## NOTAS TÉCNICAS

### Mudança na Lógica de Mensalidades

**Antes**:
```sql
WHERE rn > 2  -- Mantinha 2 mensalidades mais recentes
```

**Depois**:
```sql
WHERE rn > 1  -- Mantém apenas 1 mensalidade mais recente
```

**Impacto**: ~44 MB economizados, queries mais rápidas

### Filtros Normalizados de Banco

**Problema**: Filtro retornava "BANCO DO BRASIL" mas API buscava "BANCO DO BRASIL S.A"

**Solução**: Buscar todas as variações do nome normalizado
```python
normalize_bank_name("BANCO DO BRASIL S.A") == "BANCO DO BRASIL"
matching_entities = [e for e in all if normalize_bank_name(e) == filter]
```

### Navegação Simplificada

**Antes**: Modal → Textarea → Validação → Salvar → Navegar
**Depois**: Click → Navegar diretamente

---

## OBSERVAÇÕES FINAIS

1. **Backup Crítico**: Validar SEMPRE antes de prosseguir
2. **Mensalidades**: Sistema mantém apenas a mais recente
3. **Filtros**: Banco e Cargo agora funcionam corretamente
4. **Performance**: Banco mais leve, queries mais rápidas
5. **Compatibilidade**: 100% retrocompatível

---

**Documentado por**: Claude Code
**Commit**: 0d7484f
**Branch**: life-system
**Data**: 03/12/2025
